...
if ( $query->have_posts() ) :
    ...
    while ( $query->have_posts() ) :
        ...
        echo '<div class="brxc-modal__palette-wrapper isotope-wrapper" data-palette="' . $post_slug . '"><span class="brxc-modal__palette-title">' . get_the_title() . '</span>';
            ...
            while ( have_rows( 'brxc_colors_repeater' ) ) :
                ...
                <div class="brxc-modal__field two-icons">
                    <div class="brxc-modal__left-col">
                        <input type="color" value="<?php echo sanitize_hex_color($acf);?>" data-initial-value="<?php echo sanitize_hex_color($acf);?>" data-color="<?php echo ( isset($prefix) && $prefix ) ? strtolower( preg_replace( '/\s+/', '-', $prefix ) ) . '-' . strtolower( preg_replace( '/\s+/', '-', $label ) ) : strtolower(preg_replace('/\s+/', '-', $label));?>" onInput="BRXC.populateColors(event.target.value,event.target.dataset.color, event.target);">
                    </div>
                    <div class="brxc-modal__right-col">
                        <span><?php echo esc_attr($label);?></span>
                        <div class="brxc-modal__labels">
                            <input type="text" value="<?php echo sanitize_hex_color($acf);?>" onInput="BRXC.updateInput(event.target)">
                            <?php include BRICKS_ADVANCED_THEMER_PATH . '/inc/components/icons_light.php';?>
                        </div>
                    </div>
                </div><?php
...
if($has_colors === 0) {
    ...
    echo '<div class="brxc-modal__no-colors"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" class="brxc__svg-path"><path d="M13.05 31.9q.6 0 1.05-.45.45-.45.45-1.05 0-.6-.45-1.05-.45-.45-1.05-.45-.6 0-1.05.45-.45.45-.45 1.05 0 .6.45 1.05.45.45 1.05.45Zm-1.5-6.5h3v-9.55h-3Zm8.95 4h15.95v-3H20.5Zm0-8.55h15.95v-3H20.5ZM6.6 40q-1.2 0-2.1-.9-.9-.9-.9-2.1V11q0-1.2.9-2.1.9-.9 2.1-.9h34.8q1.2 0 2.1.9.9.9.9 2.1v26q0 1.2-.9 2.1-.9.9-2.1.9Zm0-3h34.8V11H6.6v26Zm0 0V11v26Z"/></svg>No color palettes found.</div>';
}
        ...
        while ( $query->have_posts() ) :
            ...
            if ( have_rows( 'brxc_colors_repeater' ) ) :
                ...
                while ( have_rows( 'brxc_colors_repeater' ) ) :
                    ...
                    ( isset($prefix) && $prefix ) ? $tempvalue = Advanced_Themer_Bricks\AT__Helpers::get_hex_value_from_json( '--' . strtolower( preg_replace( '/\s+/', '-', $prefix ) ) . '-' . strtolower( preg_replace('/\s+/', '-', $label ) ), '', 'light' ) : $tempvalue = Advanced_Themer_Bricks\AT__Helpers::get_hex_value_from_json( '--' . strtolower( preg_replace('/\s+/', '-', $label ) ), '', 'light' );
                    ...
                    <div class="a-container">
                        ...
                        <div class="a-panel">
                            ...
                            if ( isset( $shades ) && $shades == true){
                                ...
                                foreach ( $colors['light'] as $rows ){

                                    foreach ( $rows as $sufix => $percent ){

                                        ( isset($prefix) && $prefix ) ? $tempvalue = Advanced_Themer_Bricks\AT__Helpers::get_hex_value_from_json( '--' . strtolower( preg_replace( '/\s+/', '-', $prefix ) ) . '-' . strtolower( preg_replace('/\s+/', '-', $label ) ), $sufix, 'light' ) : $tempvalue = Advanced_Themer_Bricks\AT__Helpers::get_hex_value_from_json( '--' . strtolower( preg_replace('/\s+/', '-', $label ) ), $sufix, 'light' );
