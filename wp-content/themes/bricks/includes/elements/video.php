...
class Element_Video extends Element {
	...
	public function render() {
		...
		$has_overlay = ! empty( $settings['overlay'] ) || ! empty( $this->theme_styles['overlay'] );
	...
	/**
	 * Get the custom preview image. Similar to get_normalized_image_settings() in the image element.
	 *
	 * @since 1.7.2
	 */
	public function get_preview_image() {
		...
		$preview_image = $preview_image_type === 'custom' && ! empty( $this->settings['previewImageCustom'] ) ? $this->settings['previewImageCustom'] : false;

		// Return preview image (if we already got the preview image and the URL is not empty)
		if ( $preview_image && ! empty( $preview_image['url'] ) ) {
			return $preview_image;
		}

		// STEP: Set image size
		$preview_image['size'] = ! empty( $preview_image['size'] ) ? $preview_image['size'] : BRICKS_DEFAULT_IMAGE_SIZE;

		// STEP: Image ID or URL from dynamic data
		if ( ! empty( $preview_image['useDynamicData'] ) ) {
			$preview_images = $this->render_dynamic_data_tag( $preview_image['useDynamicData'], 'image', [ 'size' => $preview_image['size'] ] );

			if ( ! empty( $preview_images[0] ) ) {
				if ( is_numeric( $preview_images[0] ) ) {
					$preview_image['id'] = $preview_images[0];
				} else {
					$preview_image['url'] = $preview_images[0];
				}
			} else {
				return;
			}
		...
		// Set image ID
		$preview_image['id'] = empty( $preview_image['id'] ) ? 0 : $preview_image['id'];

		// Set image URL
		if ( ! isset( $preview_image['url'] ) ) {
			$preview_image['url'] = ! empty( $preview_image['id'] ) ? wp_get_attachment_image_url( $preview_image['id'], $preview_image['size'] ) : false;
		} else {
			$preview_image['url'] = $this->render_dynamic_data( $preview_image['url'] );
		}
