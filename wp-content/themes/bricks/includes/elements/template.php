...
class Element_Template extends Element {
	...
	public function render() {
		...
		if ( ! array_key_exists( $template_id, $rendered_templates ) ) {
			 ...
			 * When:
			 *
			 * 1. exteral files are used OR
			 * 2. Is 'popup' template to add .brx-popup, .brx-popup-content, etc. OR (@since 1.6)
			 * 3. Is builder call (template element in header/footer template)
			 *
			 * @since 1.5.7
			 */
			if ( ( Database::get_setting( 'cssLoading' ) === 'file' && bricks_is_frontend() ) || $is_popup_template || bricks_is_builder_call() ) {
