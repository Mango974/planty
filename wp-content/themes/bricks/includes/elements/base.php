...
abstract class Element {
	...
	public function set_root_attributes() {
		...
		if ( Query::is_looping() ) {
			...
			$query_settings = isset( Query::get_query_object()->settings['query'] ) ? Query::get_query_object()->settings['query'] : [];

			// Loop index equals offset: Reset loop index to 0 to get 'loopElementId' in Vue (@since 1.7.1)
			if ( isset( $query_settings['offset'] ) ) {
				$loop_index = $query_settings['offset'] == $loop_index ? 0 : $loop_index;
			}
			...
			// Add background image via inline style
			if ( $url ) {
			...
			// NOTE: No longer in use as it prevents _gradient in builder (@since 1.7)
	...
	public function set_link_attributes( $attribute_key, $link_settings ) {
		if ( isset( $link_settings['type'] ) ) {
			...
			$lightbox_width  = ! empty( Theme_Styles::$active_settings['general']['lightboxWidth'] ) ? Theme_Styles::$active_settings['general']['lightboxWidth'] : 600;
			$lightbox_height = ! empty( Theme_Styles::$active_settings['general']['lightboxHeight'] ) ? Theme_Styles::$active_settings['general']['lightboxHeight'] : 600;
	...
	public function generate_mobile_menu_inline_css( $settings = [], $breakpoint = '' ) {
		...
		if ( $breakpoint_width ) {
			...
			if ( $this->name === 'nav-menu' ) {
				...
				$nav_menu_inline_css .= "#{$element_id} .brx-nav-nested-items { display: none; }\n";
