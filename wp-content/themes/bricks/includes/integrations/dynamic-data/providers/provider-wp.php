	...
	public function get_tag_value( $tag, $post, $args, $context ) {
		...
		switch ( $render ) {
			...
			case 'post_excerpt':
				...
				if ( \Bricks\Query::is_looping() ) {
					...
					if ( ! is_a( $loop_object, 'WP_Post' ) ) {
						...
						if ( $loop_object && ! empty( $loop_object->description ) ) {
							$value = \Bricks\Helpers::trim_words( $loop_object->description, $num_words, '', $keep_html );
						}
				...
				elseif ( is_archive() ) {
					...
					if ( $queried_object && ! empty( $queried_object->description ) && $element_name !== 'posts' ) {
						$value = \Bricks\Helpers::trim_words( $queried_object->description, $num_words, '', $keep_html );
				...
				$value = \Bricks\Helpers::get_the_excerpt( $post, $num_words, '', $keep_html );
				break;
			...
			case 'featured_image_tag':
				...
				/**
				 * Get shop page ID for {featured_image} DD on product archive template (@since 1.7)
				 *
				 * Do not use shop page ID inside a Query Loop (@since 1.7.1)
				 */
				if ( function_exists( 'is_shop' ) && is_shop() && ! isset( $GLOBALS['woocommerce_loop'] ) && ! \Bricks\Query::is_looping() ) {
					$shop_page_id = wc_get_page_id( 'shop' );
					$value        = get_post_thumbnail_id( $shop_page_id );
				}
			...
			case 'author_avatar':
				 ...
				 * Otherwise use $post_id which should be retrived from get_queried_object_id()
				 *
				 * @since 1.7.1
				 */
