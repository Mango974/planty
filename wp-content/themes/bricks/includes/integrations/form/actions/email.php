...
class Email extends Base {
	...
	public function run( $form ) {
		...
		if ( isset( $form_settings['replyToEmail'] ) ) {
			...
			// Use first valid email address found in submitted form data as 'Reply-To' email address (is also default for confirmation email @since 1.7.2)
		...
		if ( $confirmation_email_content && $reply_to_email_address ) {
			...
			// Header: 'From'
			$confirmation_from_name  = isset( $form_settings['confirmationFromEmail'] ) ? $form->render_data( $form_settings['confirmationFromEmail'] ) : get_bloginfo( 'name' );
			$confirmation_from_email = isset( $form_settings['confirmationFromName'] ) ? $form->render_data( $form_settings['confirmationFromName'] ) : get_option( 'admin_email' );

			$confirmation_email_headers = [ "From: $confirmation_from_email <$confirmation_from_name>" ];
