...
class Query {
	...
	public function run() {
		...
		// Allow other query providers to return a query result (Woo Cart, ACF, Metabox...)
			...
			// @see https://academy.bricksbuilder.io/article/filter-bricks-query-run/
	...
	public function run_wp_user_query() {
		...
		$disable_query_merge = isset( $this->query_vars['disable_query_merge'] );
	...
	public function run_wp_query() {
		...
		if ( isset( $this->query_vars['post_parent'] ) ) {
			$post_parent = bricks_render_dynamic_data( $this->query_vars['post_parent'] );

			if ( strpos( $post_parent, ',' ) !== false ) {
				$post_parent = explode( ',', $post_parent );

				// @since 1.7.1
				$this->query_vars['post_parent__in'] = (array) $post_parent;

				unset( $this->query_vars['post_parent'] );
			} else {
				$this->query_vars['post_parent'] = (int) $post_parent;
			}
		}
		 ...
		 */
		if ( $merge_query && ( is_archive() || is_author() || is_search() || is_home() ) && empty( $this->query_vars['disable_query_merge'] ) ) {
			global $wp_query;
