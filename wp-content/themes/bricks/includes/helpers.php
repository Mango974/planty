...
class Helpers {
	...
	public static function get_template_setting( $key, $post_id ) {
		/**
		 * Return: Don't use any 'templatePreview' template settings on the frontend
		 *
		 * Except for the template preview itself (post type: bricks_template)
		 *
		 * @since 1.7.1
		 */
		if ( bricks_is_frontend() && get_post_type( $post_id ) !== BRICKS_DB_TEMPLATE_SLUG ) {
			if ( strpos( $key, 'templatePreview' ) === 0 ) {
				return '';
			}
		}
	...
	public static function trim_words( $text, $length, $more = null, $keep_html = false ) {
		...
		$more = isset( $excerpt_more ) ? $more : '&hellip;';
