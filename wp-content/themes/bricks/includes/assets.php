...
class Assets {
	...
	public static function enqueue_setting_specific_scripts( $settings = [] ) {
		...
		// STEP: Load Photoswipe for any lightbox setting (also lightboxImage, lightboxVideo, etc.)
		if ( strpos( $bricks_settings_string, '"lightbox' ) !== false ) {
		...
		// STEP: Generate inline style
		$inline_style = '';

		// Generate dynamic data CSS for requested page inline
		if ( ! bricks_is_builder() && self::$inline_css_dynamic_data && empty( $settings ) ) {
			$inline_style .= self::$inline_css_dynamic_data;
		}

		// Bricks settings: Smooth scroll (CSS) (@since 1.7.1)
		if ( Database::get_setting( 'smoothScroll' ) ) {
			$inline_style .= 'html {scroll-behavior: smooth}';
		}

		if ( $inline_style ) {
			wp_add_inline_style( 'bricks-frontend', $inline_style );
		}
	}
	...
	public static function load_webfonts( $inline_css ) {
		...
		foreach ( $google_fonts_families as $google_font ) {
			...
			if ( in_array( $google_font['family'], $adobe_fonts_in_use ) ) {
				continue;
			}
		...
		if ( $css_definitions ) {
			foreach ( $control['css'] as $css_definition ) {
				...
				if ( isset( $css_definition['value'] ) ) {
					...
					// 'required' value set, but doesn't match $setting_value: Skip adding rule (@since 1.7.2)
					...
					switch ( $control_type ) {
							...
							foreach ( $setting_value as $background_property => $background_value ) {
								switch ( $background_property ) {
									...
									case 'image':
										...
										/**
										 * Add dynamic data background image, if ...
										 *
										 * (1) pseudo class OR
										 * (2) not inside query loop (where its already added via inline style, @see base.php L1300) OR
										 * (3) Video element overlay image (#862hxy9mx, @since 1.6.1)
										 */

										if ( $background_url ) {
											$is_video_overlay = strpos( $setting_key, 'overlay' ) === 0;

											if ( $pseudo_class || ! Query::is_looping() || $is_video_overlay ) {
												...
												if ( $dynamic_tag && Database::get_setting( 'cssLoading' ) === 'file' ) {
													$dynamic_data_background = $css_selector . ' {background-image: url(' . esc_url( $background_url ) . ')} ';

													// Add at-media rule for breakpoint (@since 1.7.2)
													if ( $breakpoint ) {
						...
						case 'border':
							...
							if ( count( $border_width_values ) ) {
								...
								if ( count( $border_width_values ) === 4 && count( array_unique( $border_width_values ) ) === 1 ) {
									...
									elseif ( $border_style ) {
										...
										$css_rules[ $css_selector ][] = "border: {$border_width_values[0]} $border_style $border_color";
							...
							if ( $border_color && ! $border_color_set ) {
								$css_rules[ $css_selector ][] = "border-color: $border_color";
							}
	...
	public static function get_at_media_rule_for_breakpoint( $bp ) {
		...
		foreach ( $breakpoints as $breakpoint ) {
			...
			if ( ! $is_base_breakpoint ) {
				$at_media_rule = $width > $base_width ? "@media (min-width: {$width}px) {\n" : "@media (max-width: {$width}px)";
			}
	...
	public static function generate_css_from_elements( $elements, $css_type ) {
		...
		foreach ( $elements as $element ) {
			...
			/**
			 * STEP: External files: Skip global elements from being added to the post-specific CSS file
			 *
			 * As we already serve them in the global-elements.min.css file.
			 *
			 * NOTE: Not in use as it doesn't generate post-specific CSS styles for nested elements or inside loops (@see #8677qz1b9)
			 *
			 * @since 1.7.1
			 */
			// if ( Database::get_setting( 'cssLoading' ) === 'file' && Helpers::get_global_element( $element ) ) {
			// continue;
			// }
